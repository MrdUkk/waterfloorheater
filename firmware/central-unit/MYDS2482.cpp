...
MYDS2482::MYDS2482(uint8_t addr)
{
        mAddress = 0x18 | addr;
}
...
void MYDS2482::setReadPtr(uint8_t readPtr)
{
        begin();
...
uint8_t MYDS2482::readByte()
{
        Wire.requestFrom(mAddress,(uint8_t)1);
        return Wire.read();
}

uint8_t MYDS2482::wireReadStatus(bool setPtr)
{
        if (setPtr)
...
uint8_t MYDS2482::busyWait(bool setReadPtr)
{
        uint8_t status;
        int loopCount = 1000;
...
uint8_t MYDS2482::reset()
{
        ...
        return status & DS2482_STATUS_RST ? true : false;
}

bool MYDS2482::configure(uint8_t config)
{
        busyWait(true);
        begin();
        Wire.write(0xd2);
...
bool MYDS2482::selectChannel(uint8_t channel)
{
        uint8_t ch, ch_read;
...
void MYDS2482::wireWriteByte(uint8_t b)
{
        busyWait(true);
        begin();
        Wire.write(0xa5);
...
void MYDS2482::wireWriteBit(uint8_t bit)
{
        busyWait(true);
        begin();
        Wire.write(0x87);
...
void MYDS2482::wireSkip()
{
        wireWriteByte(0xcc);
}

void MYDS2482::wireSelect(const uint8_t rom[8])
{
        wireWriteByte(0x55);
        for (int i=0;i<8;i++)
                wireWriteByte(rom[i]);
...
uint8_t MYDS2482::wireSearch(uint8_t *newAddr)
{
        uint8_t i;
        uint8_t direction;
        uint8_t last_zero=0;
...
uint8_t MYDS2482::devicesCount(bool printAddress){
  uint8_t address[8];
  uint8_t count = 0;
  String SerialNumber = "";
...
uint8_t MYDS2482::crc8(const uint8_t *addr, uint8_t len)
{
        uint8_t crc=0;
